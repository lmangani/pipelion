...
function invoke(fnName, args, debug) {
  if(debug)console.log('invoking', fnName, args[0], args[1])
  if(run[fnName]) {
    ...
    if(isAsync) {
        ...
        var promise = new  Promise (function(resolve, reject) {
            if(debug)console.log('called with args', args[0], args[1]);
            let result = run[fnName](args[0], args[1]);
    ...
    var promise = new Promise (function(resolve, reject) {
        if(debug)console.log('sending finalized result back as promise', args[0], args[1])
        resolve(args[0]);
    })
    return promise;
  }
