...
function invoke(fnName, args, debug) {
  ...
  if(debug)console.log('invoking', fnName, input, args)
  if(run[fnName]) {
    var isAsync = run[fnName].constructor.name == 'AsyncFunction';
    if(debug)console.log(fnName + ' isAsync ' + isAsync)
    if(isAsync) {
        return run[fnName](input, args);
    } else {
        var promise = new  Promise (function(resolve, reject) {
            if(debug)console.log('called with args', input, args);
            let result = run[fnName](input, args);
    ...
    var promise = new Promise (function(resolve, reject) {
        if(debug)console.log('sending finalized result back as promise', input, args)
        resolve(input);
    })
    return promise;
  }
